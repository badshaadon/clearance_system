    ...
    <?php
      if ((isset($_POST['accept'])) || (isset($_POST['reject']))) {
        ...
        if((isset($_POST['accept'])) && (mysqli_num_rows($result) == 0)) {
          $query    = "INSERT into `request_status` (username, account) VALUES ('$user_name' ,'accept')";
          ...
          $sql = "UPDATE `request_status` SET `account` = 'accept' WHERE `username` = '$user_name'";
        ...
        if((isset($_POST['reject'])) && (mysqli_num_rows($result) == 0)) {
          $query    = "INSERT into `request_status` (username, account) VALUES ('$user_name' ,'rejected')";
          $result   = mysqli_query($con, $query) or die("can't add record" . mysqli_error($con));
          if ($result) {
          ...
          $sql = "UPDATE `request_status` SET `account` = 'reject' WHERE `username` = '$user_name'";
          if (mysqli_query($con, $sql)) {
