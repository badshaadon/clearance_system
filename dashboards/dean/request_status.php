      ...
      <?php
        if ((isset($_POST['accept'])) || (isset($_POST['reject']))) {
          ...
          if((isset($_POST['accept'])) && (mysqli_num_rows($result) == 0)) {
            $query    = "INSERT into `request_status` (username, `dean`) VALUES ('$user_name' ,'accept')";
            $result   = mysqli_query($con, $query) or die("can't add record" . mysqli_error($con));
            if ($result) {
              echo "<div class='jumbotron bg-secondary text-center my-3 py-5 text-warning'>
            ...
            $sql = "UPDATE `request_status` SET `dean` = 'accept' WHERE `username` = '$user_name'";
          ...
          if((isset($_POST['reject'])) && (mysqli_num_rows($result) == 0)) {
            $query    = "INSERT into `request_status` (username, `dean`) VALUES ('$user_name' ,'rejected')";
            $result   = mysqli_query($con, $query) or die("can't add record" . mysqli_error($con));
            if ($result) {
              echo "<div class='jumbotron bg-secondary text-center my-3 py-5 text-warning'>
            ...
            $sql = "UPDATE `request_status` SET `dean` = 'reject' WHERE `username` = '$user_name'";
            if (mysqli_query($con, $sql)) {
